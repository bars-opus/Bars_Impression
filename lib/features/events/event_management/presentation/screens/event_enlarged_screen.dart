...
class _EventEnlargedScreenState extends State<EventEnlargedScreen>
  ...
  void initState() {
    ...
    _setUpCommentCount();
    _setUpLikesCount();
    _displayReportWarning = widget.post.report.isNotEmpty;
    // _setUpTicket();
  ...
  _setUpLikesCount() async {
    try {
      DatabaseService.numLikes(widget.post.id!).listen((activityCount) {
        if (mounted) {
          setState(() {
            _dbLikeCount = activityCount;
          });
        }
      });
    } catch (e) {}
  ...
  _setUpCommentCount() async {
    try {
      DatabaseService.numComments(widget.post.id!).listen((activityCount) {
        if (mounted) {
          setState(() {
            _dbCommentCount = activityCount;
          });
        }
      });
    } catch (e) {}
  ...
  _likePost() {
    var _provider = Provider.of<UserData>(context, listen: false);
    try {
      DatabaseService.likePost(user: _provider.user!, post: widget.post);
      if (mounted) {
        setState(() {
          _isLiked = true;
          _dbLikeCount = _dbLikeCount + 1;
...
// //build ask question display
      ...
      onSend: () {
        ...
        if (trimmedText.isNotEmpty) {
          try {
            ...
            _showBottomSheetErrorMessage('Failed to send comment.',
  ...
  void _showBottomSheetAskMore(BuildContext context) async {
    bool _isAuthor = widget.currentUserId == widget.post.authorId;
                                    ...
                                    child: Center(
                                      child: NoContents(
                                        ...
                                        subTitle: _isAuthor
                                            ...
                                            : ' You can be the first to comment on this post. Engage with others and make the experience interactive!',
                                      ),
                                    ),
                                  )
                                : Container(
                                          ...
                                          slivers: [
                                            SliverList(
                                              delegate:
                                                ...
                                                (context, index) {
                                                  CommentModel comment =
                                                      CommentModel.fromDoc(
                                                          snapshot.data
                                                              .docs[index]);
                                                  return _buildAsk(comment);
                                                },
                                                childCount:
            ...
            _isAuthor
                ...
                : GestureDetector(
                    ...
                    child: Row(
                      ...
                      crossAxisAlignment: CrossAxisAlignment.start,
                      children: [
                        widget.post.authorIdProfileImageUrl.isEmpty
                            ? const Icon(
                            ...
                            : CircleAvatar(
                                ...
                                backgroundImage: CachedNetworkImageProvider(
                                    widget.post.authorIdProfileImageUrl,
                                    errorListener: (_) {
                                  return;
                                }),
                              ),
                        Container(
                          ...
                          child: Column(
                            ...
                            children: [
                              ...
                              Text(
                                widget.post.storeType.replaceAll('\n', ' '),
                                style: TextStyle(
                                  fontSize: ResponsiveHelper.responsiveFontSize(
                                      context, 10.0),
                                  color: Colors.blue,
      ...
      body: Stack(
        ...
        children: <Widget>[
          Container(
            ...
            width: double.infinity,
            child: BlurHash(
              hash: widget.post.blurHash.isEmpty
                  ? 'LpQ0aNRkM{M{~qWBayWB4nofj[j['
                  : widget.post.blurHash,
          ...
          Text(
            ...
            style: TextStyle(
              fontSize: ResponsiveHelper.responsiveFontSize(context, 50.0),
